...
\begin{enumerate}[noitemsep]
    ...
    \item \hyperref[sec:limit_2]{Второй замечательный предел}
\end{enumerate}
...
\section{Прямая на плоскости: уравнение через две точки, каноническое, параметрическое, общее, в отрезках на осях}\label{sec:line}
...
Рассмотрим односторонние пределы \(\lim_{x \to +0}{\dfrac{\sin{x}}{x}}\) и \(\lim_{x \to -0}{\dfrac{\sin{x}}{x}}\) и докажем, что они равны \(1\).
...
\section{Второй замечательный предел}\label{sec:limit_2}
\begin{align*}
     &  &
